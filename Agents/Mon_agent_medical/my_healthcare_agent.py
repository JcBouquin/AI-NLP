...
async def main():
    async with p.Server() as server:
        ...
        # 1. Utiliser QNA UNIQUEMENT pour infos spécifiques au cabinet
        # Note: Le tool 'qna' est disponible via --module parlant_qna.module
        # Pour les connaissances médicales générales, le LLM répond directement (plus rapide)
        await agent.create_guideline(
            condition="Le patient demande des informations spécifiques au cabinet (horaires, médecins, tarifs, adresse, politique)",
            action="Search the cabinet's information knowledge base",
            tools=["qna"]  # ← Appelle le RAG pour données du cabinet uniquement
        )
