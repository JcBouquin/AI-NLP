...
@mcp.prompt()
def analyze_legal_delegations(text_content: str) -> str:
    """
    Prompt d'analyse sophistiquée pour identifier les délégations de pouvoir dans les documents juridiques.
    
    Args:
        text_content (str): Texte extrait du document à analyser
    Returns:
        str: Prompt formaté pour l'analyse
    """
    return f"""Vous êtes un expert en analyse de texte juridique et administratif.
...
# ===== PARTIE 3: ANALYSE AVEC LE PROMPT MCP =====

def analyze_extracted_file(extracted_file_path: str):
    """
    Analyse le fichier extrait avec le prompt MCP
    """
    try:
        # Lire le fichier
        ...
        # Utiliser le prompt MCP au lieu du template en dur
        prompt_text = analyze_legal_delegations(text_content)
        
        # Créer le template pour langchain avec le prompt MCP
        analysis_prompt_template = ChatPromptTemplate.from_messages([
            ("human", prompt_text)
        ])
        
        # Préparer et invoquer le LLM
...
"""
        ...
        # Étape 2: Analyse avec le prompt MCP
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--test":
        ...
        print("📝 Prompt MCP disponible:")
        print("   - analyze_legal_delegations: Prompt sophistiqué")
